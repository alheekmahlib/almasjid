...
/// عميل الشبكة الموحد مع دعم رابط بديل (GitLab) عند فشل الرابط الأساسي
/// (GitHub) — منقول من تطبيق القرآن الكريم.
class ApiClient {
  final Dio _dio;
    ...
    : _dio = Dio(
        BaseOptions(
          ...
          validateStatus: (status) {
            ...
            // Allow handling all status codes
  ...
  Future<Either<Failure, dynamic>> request({
    ...
    try {
      ...
      // إذا فشل الطلب ووجد رابطاً بديلاً، حاول GitLab
      if (fallbackUrl != null) {
        return _requestFallback(fallbackUrl, headers);
      }
  ...
  Future<Either<Failure, dynamic>> _requestFallback(
    ...
    try {
      ...
      log(
        'Fallback also failed: ${e.message}, Status Code: ${e.response?.statusCode}',
        name: 'ApiClient',
      );
      return Left(ErrorHandler.handle(e).failure);
    } catch (e) {
  ...
  /// مرر [options] مع ResponseType.bytes للملفات الصوتية/الثنائية.
  Future<Either<Failure, dynamic>> downloadFile({
    ...
    try {
      ...
      // إذا فشل التحميل ووجد رابطاً بديلاً، حاول منه
      if (fallbackUrl != null) {
        return _downloadFallback(fallbackUrl, onProgress, timeout, options);
      }
